...
## 从 Point_TL 出发的冲刺距离
const DASH_DISTANCE_TL: float = 400.0
## 从 Point_TR 出发的冲刺距离
const DASH_DISTANCE_TR: float = 250.0
## 冲刺速度
const DASH_SPEED: float = 650.0

const POINT_NAMES: Array[String] = ["Point_TL", "Point_TR"]
...
func _execute_dash() -> void:
	...
	boss.animated_sprite.play("down")
	...
	_dash_distance = DASH_DISTANCE_TL if point_name == "Point_TL" else DASH_DISTANCE_TR
