...
## 第四关 BOSS 死亡导演（独立实现，不继承公共基类）
## 流程：红黑剪影定格 → 横斩 → 死亡动画 → 黑屏 → 下一章
## 差异：
##  1. BOSS4 由骷髅头 + 女人身体两个动画组成，死亡剪影需要同时显示两者
##  2. BOSS4 为建筑类 BOSS，死亡后保持原地播放死亡动画，不做下沉
...
func _capture_freeze_frame(boss: Boss_4) -> void:
...
func _disable_attack_hitboxes(boss: Boss_4) -> void:
...
func _safe_freeze_before_capture(boss: Boss_4) -> void:
	boss.is_dead = true
...
func play_death_sequence(boss: Boss_4) -> void:
	if _is_playing:
		return
	_is_playing = true
	BossDeathDirector_4.is_death_playing = true
	# 同步基类静态标志，保证 UIManager 等公共逻辑在死亡演出期间照常生效
...
func _phase_recover() -> void:
	...
	# BOSS4 为建筑类 BOSS：死亡后保持原地播放死亡动画，不做下沉
...
func _end_level() -> void:
	_is_playing = false
	BossDeathDirector_4.is_death_playing = false
