...
## ============================================================
##
##  功能：玩家走进 TriggerArea 区域时，在 SpawnPosition 位置生成一群蝙蝠。
##
##  场景结构：
##    BatSpawner (Node2D)
...
##
##  在关卡中的用法：
##    1. 把 bat_spawner.tscn 拖入关卡场景
##    2. 右键 BatSpawner 实例 → Editable Children（可编辑子节点）
##    3. 拖拽 TriggerArea 的碰撞框调整触发范围
##    4. 拖拽 SpawnPosition 十字标记到蝙蝠出现的位置
##    5. 在 Inspector 中调整生成数量、方向等参数
##    6. 改完后自动保存，无需额外操作
##
##  注意：每只蝙蝠会复制自己的 data 资源，不会污染 BatData.tres 原文件
## ============================================================


@export var spawn_count: int = 3             # 一次生成几只蝙蝠
@export var spawn_direction: int = -1        # -1 向左飞，1 向右飞
@export var spawn_offset_x: float = 16.0     # 蝙蝠之间的横向间距（像素）
@export var spawn_offset_y: float = 0.0      # 蝙蝠之间的垂直偏移（像素）
...
func _do_spawn() -> void:
	var bat_scene = preload("res://scenes/enemy/l1/bat_enemy.tscn")
	...
	for i in range(spawn_count):
		...
		bat.global_position = spawn_origin + Vector2(offset_x, spawn_offset_y)

		bat.data = bat.data.duplicate()
		bat.data.facing_right = (spawn_direction == 1)
