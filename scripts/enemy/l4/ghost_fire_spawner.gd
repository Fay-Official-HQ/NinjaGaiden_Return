# GhostFireSpawner（第四关鬼火生成器）
#
# 行为：
#   - 玩家进入触发区域后立即生成一波，之后只要玩家持续待在区域内，
#     每隔 spawn_interval 秒就持续生成一波（一次可生成多个）；
...
@export var spawn_sound: StringName = &"guihuo"  # 每生成一波时播放的音效
...
func _do_spawn() -> void:
	...
	if max_spawns >= 0 and _spawned_waves >= max_spawns:
		stop_spawning()
		return
	var scene = preload("res://scenes/enemy/l4/ghost_fire.tscn")
...
#   var spawner = get_tree().current_scene.get_node("enemys/GhostFireSpawner")
