extends Area2D
class_name MechEagle

## ============================================================
##  MechEagle —— 机械老鹰（去数据驱动版）
...
func _update_dive(delta: float) -> void:
	...
	if not _dive_locked:
		...
		_curve_velocity = _curve_velocity.lerp(
			to_player * move_speed,
			delta * dive_curve_strength * 0.01
		)
		_face_player()

		# 加速到接近最大速度时 → 锁定直线方向，不再追踪玩家
		if _curve_velocity.length() >= move_speed * 0.95:
			_dive_locked = true
			_locked_direction = _curve_velocity.normalized()
			_dive_start_position = global_position
	else:
		# 阶段2：锁定方向，直线高速俯冲（此时轨迹固定，可被跳跃躲避）
		_curve_velocity = _locked_direction * move_speed
