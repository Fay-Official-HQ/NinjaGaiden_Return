extends Node2D
class_name MechEagleSpawner

## ============================================================
##  MechEagleSpawner —— 机械老鹰生成器
##  （参照第一关 EagleSpawner，逻辑与 MechBeeSpawner 一致）
## ============================================================
##  功能：
##    - one_shot = true（默认）：玩家进入 TriggerArea 生成一波机械老鹰后，生成器消失
##    - one_shot = false：玩家进入立即生成一波；持续待在区域内时，
##      每隔 spawn_interval 秒持续生成；离开暂停、再进入恢复
##    - 机械老鹰会自动面向玩家俯冲，无需设置飞行方向
##  场景结构：
##    MechEagleSpawner (Node2D)
##    ├─ TriggerArea (Area2D)      ← 碰撞框拖成触发区域
##    └─ SpawnPosition (Marker2D)  ← 生成位置
## ============================================================

# ── Inspector 可调参数 ──
@export var spawn_count: int = 1            # 一次生成几只机械老鹰
@export var spawn_offset_x: float = 30.0    # 机械老鹰之间的横向间距（像素），以中心为基准分散
...
func _do_spawn() -> void:
	...
	var scene = preload("res://scenes/enemy/l5/mech_eagle.tscn")
