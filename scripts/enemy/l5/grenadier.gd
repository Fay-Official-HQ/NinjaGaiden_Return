# BombNinja —— 手雷敌人（下蹲炸弹忍者）
# ============================================================
#  行为模式：
#    1. 原地不动，面朝玩家
#    2. DetectRange 检测到玩家 → 持续投掷抛物线手雷（以玩家为落点）
#    3. 投掷间隔可调
# ============================================================
...
const GRENADE_SCENE = preload("res://scenes/enemy/l5/shoulei.tscn")
...
# 【调试】投掷点 Marker2D（场景中按朝右摆放），炸弹从这里生成，翻转朝向时 x 镜像
...
## 【调试】手雷水平参考速度（像素/秒），越大手雷落地越快、抛物线越平缓
@export var dart_speed: float = 250.0
## 【调试】手雷重力加速度（像素/秒²），越大抛物线越陡
...
@export var throw_sound: StringName = &"fireinthehole"
...
func _ready() -> void:
	...
	# 初始不朝右时，投掷点也需镜像（场景中 Marker2D 按朝右摆放）
	if not facing_right:
...
## 开始播放 throw 动画（手雷在最后一帧才扔出）
func _begin_throw() -> void:
	_has_thrown = false
	anim.play("throw")


## throw 动画播放到最后一帧时扔出手雷
func _on_anim_frame_changed() -> void:
	if is_dead:
		return
	if anim.animation != "throw" or _has_thrown:
		return
	var frame_count = anim.sprite_frames.get_frame_count("throw")
...
func _spawn_grenade() -> void:
	...
	# 【调试】实例化手雷并抛物线投掷
	#   - 以玩家当前位置为落点，throw_to 自动计算抛物线初速度
	#   - dart_speed：水平参考速度，决定飞行时间
	#   - grenade_gravity：手雷重力，越大弧线越陡
	#   - 出生点：Marker2D 投掷点（已随朝向镜像，左右对称）
	var dart = GRENADE_SCENE.instantiate()
	get_tree().current_scene.add_child(dart)
	dart.global_position = throw_point.global_position
	dart.throw_to(player.global_position, dart_speed, grenade_gravity)
