...
## ============================================================
##  SkyShinobiSpawner —— 飞天忍者（双形态）生成器
## ============================================================
##
##  功能：玩家走进 TriggerArea 区域时，在 SpawnPosition 位置生成 SkyShinobi。
##       可通过 gender 选择女人（飞镖）或男人（火球）。
##
##  场景结构：
##    SkyShinobiSpawner (Node2D)
...
func _do_spawn() -> void:
	var scene = preload("res://scenes/enemy/l3/SkyShinobi.tscn")
	var origin = spawn_position.global_position

	for i in range(spawn_count):
		var enemy = scene.instantiate() as SkyShinobi
