...
# ==================== 常量配置 ====================
const SOUND_DELAY: float = 0.3      # 触发后延迟播放风声（秒）
const TEXT_DELAY: float = 1.0       # 触发后延迟显示文字（秒）
const WIND_TEX = preload("res://assets/sprites/map/wind.png")

# ==================== 外部可调参数（Inspector） ====================
## 风力推玩家速度（像素/秒），范围30~200（玩家速度100）
...
func _on_body_entered(body: Node2D) -> void:
	if not body.is_in_group("player") or _state != ZoneState.IDLE:
		return
	if Time.get_ticks_msec() / 1000.0 - _last_trigger_time < cooldown:
		return
	_player = body
	_start_sequence()
